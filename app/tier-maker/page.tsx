...
export default function TierMakerPage() {
  ...
  const [communityLists, setCommunityLists] = useState<{ id: string; name: string; author: string; character_ids: number[]; created_at: string }[]>([])
  ...
  useEffect(() => {
    if (!communityBrowseOpen) return
    setCommunityLoading(true)
    fetch(`/api/community/tier-lists?page=${communityPage}&limit=20`)
      .then((r) => r.json())
      .then((json) => { setCommunityLists(json.data ?? []); setCommunityTotal(json.total ?? 0) })
      .finally(() => setCommunityLoading(false))
  }, [communityBrowseOpen, communityPage])

  const communityFiltered = useMemo(() => {
    if (!communitySearch.trim()) return communityLists
    const q = communitySearch.toLowerCase()
    return communityLists.filter((l) => l.name.toLowerCase().includes(q) || l.author.toLowerCase().includes(q))
  }, [communityLists, communitySearch])

  function openCommunityList(item: { id: string }) {
    fetch(`/api/community/tier-lists/${item.id}`)
      .then((r) => r.json())
      .then((json) => {
        const data = json.data
        if (!data) return
        const encoded = encodeUnicodeToBase64(JSON.stringify(data))
        const url = `/tier-maker?d=${encodeURIComponent(encoded)}`
  ...
  async function shareToCommunity() {
    ...
    try {
      const payload = tierLists.length === 1 ? { lists: [{ name: tierLists[0]?.name ?? "Tier List", tiers }] } : { lists: tierLists }
      const res = await fetch("/api/community/tier-lists", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, author: shareAuthor.trim() || "Anonymous", data: payload }),
      })
      const json = await res.json()
      ...
      setShareName("")
      setShareAuthor("")
      alert("Shared successfully! It will appear on the Community Tier Lists page.")
    } catch (e) {
      ...
      <div className="max-w-7xl mx-auto pl-6 pr-4 sm:pl-8 sm:pr-6 lg:px-8 py-8">
        ...
        <div className="mb-4 flex flex-wrap gap-2 items-center">
          ...
          <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
            <DialogTrigger asChild>
              <button className="px-3 py-1 rounded bg-blue-700 text-white hover:bg-blue-600 border border-blue-600 text-sm">Share to Community</button>
            </DialogTrigger>
            <DialogContent className="w-96 bg-[#0b1220] text-white border border-gray-700">
              <DialogHeader>
                <DialogTitle>Share to Community</DialogTitle>
                <DialogDescription>Share your tier list(s) so other players can browse and open them.</DialogDescription>
              </DialogHeader>
              <div className="mt-4 space-y-3">
                <div>
                  <label className="text-sm text-gray-300 mb-1 block">Name <span className="text-red-400">*</span></label>
                  <Input value={shareName} onChange={(e) => setShareName(e.target.value)} placeholder="e.g. My PvE Tier List" className="h-10 rounded-md border border-gray-700 bg-[#232c3a] px-3 text-white placeholder:text-gray-400" />
                </div>
                <div>
                ...
                <p className="text-xs text-gray-500">Sharing {tierLists.length} tier list{tierLists.length !== 1 ? "s" : ""}.</p>
              </div>
              <DialogFooter>
                <DialogClose asChild><button className="px-3 py-1.5 bg-gray-700 text-white rounded text-sm">Cancel</button></DialogClose>
                <button onClick={shareToCommunity} disabled={shareSubmitting} className="px-3 py-1.5 bg-blue-600 text-white rounded text-sm disabled:opacity-50 hover:bg-blue-500">{shareSubmitting ? "Sharing..." : "Share"}</button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          {/* Browse Community Tier Lists */}
          <Dialog open={communityBrowseOpen} onOpenChange={(o) => { setCommunityBrowseOpen(o); if (o) setCommunityPage(1) }}>
            <DialogTrigger asChild>
              <button className="px-3 py-1 rounded bg-purple-800 text-white hover:bg-purple-700 border border-purple-700 text-sm">Browse Community</button>
            </DialogTrigger>
            <DialogContent className="max-w-3xl w-[95vw] max-h-[85vh] flex flex-col bg-[#0b1220] text-white border border-gray-700 overflow-hidden">
              <DialogHeader className="shrink-0">
                <DialogTitle>Community Tier Lists</DialogTitle>
                <DialogDescription>Browse tier lists shared by other players. Click to open in a new tab.</DialogDescription>
              </DialogHeader>
              <div className="flex flex-col gap-3 overflow-hidden flex-1 min-h-0">
                ...
                <div className="overflow-y-auto flex-1 min-h-0">
                  {communityLoading ? (
                    ...
                    <div className="py-10 text-center text-gray-500 text-sm">No tier lists found yet. Be the first to share one!</div>
                  ) : (
                    <div className="grid gap-3 sm:grid-cols-2">
                      {communityFiltered.map((list) => (
                        <div key={list.id} className="flex flex-col gap-2 rounded-xl border border-gray-700 bg-gray-800 p-3">
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <p className="font-semibold text-white text-sm truncate">{list.name}</p>
                              <p className="text-xs text-gray-400">by {list.author}</p>
                            </div>
                          ...
                          {list.character_ids?.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {list.character_ids.slice(0, 14).map((id) => {
                                const c = allChars.find((x) => x.master_pc_id === id)
                                if (!c) return null
                                return <div key={id} className="w-7 h-7 rounded bg-gray-700 overflow-hidden"><img src={c.images.icon} alt={c.name} title={c.name} className="w-full h-full object-cover object-top" /></div>
                              })}
                              {list.character_ids.length > 14 && <span className="text-xs text-gray-500 self-center">+{list.character_ids.length - 14}</span>}
                            </div>
                          )}
                          <button onClick={() => openCommunityList(list)} className="mt-auto w-full rounded-lg bg-blue-600 py-1.5 text-xs font-semibold text-white hover:bg-blue-500 transition-colors">
                            Open in Tier Maker
                          </button>
                        </div>
                      ))}
                ...
                {Math.ceil(communityTotal / 20) > 1 && (
                  <div className="flex items-center justify-center gap-3 pt-1 shrink-0">
                    <button onClick={() => setCommunityPage((p) => Math.max(1, p - 1))} disabled={communityPage === 1} className="px-3 py-1 rounded bg-gray-700 text-xs disabled:opacity-40 hover:bg-gray-600">Previous</button>
                    <span className="text-xs text-gray-400">{communityPage} / {Math.ceil(communityTotal / 20)}</span>
                    <button onClick={() => setCommunityPage((p) => Math.min(Math.ceil(communityTotal / 20), p + 1))} disabled={communityPage === Math.ceil(communityTotal / 20)} className="px-3 py-1 rounded bg-gray-700 text-xs disabled:opacity-40 hover:bg-gray-600">Next</button>
                  </div>
                )}
              </div>
            </DialogContent>
